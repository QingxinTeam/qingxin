...
public class LDADataset implements Serializable{
	...
	public static LDADataset readDataSet(String filename){
		try {
			List<Map<String, Object>> reader=LdaService.queryData(LdaService.FENCI_INDEX_NAME);
	 ...
	 *  @return dataset if success and null otherwise
	 */
	public static LDADataset readDataSet(List<Map<String, Object>> reader){
		try {
			...
			for (int i = 0; i < M; ++i){
				line = (String) reader.get(i).get("line");
				////System.out.println(line);
				data.setDoc(line, i);
			}
